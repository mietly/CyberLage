...
export default async function handler(req, res) {
  ...
  console.log(`Resend key prefix: ${resendKey ? resendKey.substring(0, 10) + '...' : 'MISSING'}`)

  // 1. Send confirmation mail via Resend
  if (resendKey) {
    try {
      // Use verified domain, fallback to resend test address
      ...
      const payload = {
        ...
        html: `
          <div style="font-family: Arial, Helvetica, sans-serif; max-width: 600px; margin: 0 auto; background: #0A0C0F; color: #E8E6E0; padding: 40px 30px;">
            <div style="border-bottom: 2px solid #C8A96E; padding-bottom: 20px; margin-bottom: 30px;">
              <h1 style="font-size: 24px; margin: 0; color: #E8E6E0;">&#128737; CyberLage</h1>
            ...
            <div style="border-top: 1px solid #1E2228; margin-top: 30px; padding-top: 20px;">
              <p style="color: #3E4148; font-size: 11px;">
                CyberLage.io &middot; Betrieben von Corelead Solutions &middot; München, Deutschland
              </p>
            </div>
          </div>
        `,
      ...
      console.log(`Sending mail to ${email} from ${fromAddress}`)
      ...
      if (resendRes.ok && resendData.id) {
        mailSent = true
        console.log(`SUCCESS: Mail gesendet an ${email}, ID: ${resendData.id}`)
      } else {
        mailError = resendData.message || resendData.name || JSON.stringify(resendData)
        console.error(`FAILED: Mail an ${email}: ${mailError}`)
      }
    } catch (e) {
      mailError = e.message
      console.error(`EXCEPTION: Mail an ${email}:`, e.message)
    }
  } else {
    mailError = 'Kein Resend API Key konfiguriert. Setze RESEND_API_KEY in Vercel Environment Variables.'
    console.error('RESEND_API_KEY fehlt! Weder RESEND_API_KEY noch VITE_RESEND_API_KEY gesetzt.')
  }

  // 2. Save subscriber to Supabase
  ...
  if (supabaseUrl && supabaseKey) {
    try {
      ...
      if (dbError) {
        ...
        dbSaved = true
        console.log(`Subscriber ${email} in Supabase gespeichert`)
      }
    } catch (e) {
  ...
  return res.status(200).json({
    ...
    message: mailSent
      ...
      : `Anmeldung gespeichert. Mail-Status: ${mailError || 'unbekannt'}`,
  })
}
